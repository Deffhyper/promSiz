<?php include_once "parts/head.php" ?>
<body>
<?php include_once "parts/header.php" ?>

<?php include_once "parts/breadcrumbs.php" ?>

<section class="reviews-page">
    <div class="container">
        <ul class="three-list list-inline">
            <li class="three-list__item">
                <div class="reviews-list__top--wrapper">
                    <a href="#" class="reviews-list__item--image" data-toggle="modal" data-target="#rev-1">
                        <svg class="icon" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="36" height="36" viewBox="0 0 36 36" style="">
                            <image id="Shape_135_copy" data-name="Shape 135 copy" width="36" height="36" xlink:href="data:img/png;base64,"/>
                        </svg>
                        <img src="img/jpeg_files/rev-1.jpg" alt="document">
                    </a>
                </div>
                <div class="reviews-list__bottom--wrapper">
                    <a href="#" class="reviews-list__item--title">Отзыв от компании “ТехноБУД”</a>
                </div>

                <div id="rev-1" class="modal fade image-modal" role="dialog">
                    <div class="modal-dialog">
                        <!-- Modal content-->
                        <div class="modal-content">
                            <div class="modal-header">
                                <a class="close" data-dismiss="modal"><i class="icon-sprite icon-close"></i></a>
                            </div>
                            <div class="modal-body">
                                <img src="img/jpeg_files/rev-1-big.jpg" alt="document">
                            </div>
                            <div class="modal-footer">
                                <p class="reviews-modal-subtitle">Сертификат соответствия всем нормам и стандартам при разработки СИЗ</p>
                            </div>
                        </div>
                    </div>
                </div>
            </li>
            <li class="three-list__item">
                <div class="reviews-list__top--wrapper">
                    <a href="#" class="reviews-list__item--image" data-toggle="modal" data-target="#rev-2">
                        <svg class="icon" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="36" height="36" viewBox="0 0 36 36" style="">
                            <image id="Shape_135_copy" data-name="Shape 135 copy" width="36" height="36" xlink:href="data:img/png;base64,"/>
                        </svg>
                        <img src="img/jpeg_files/rev-2.jpg" alt="document">
                    </a>
                </div>
                <div class="reviews-list__bottom--wrapper">
                    <a href="#" class="reviews-list__item--title">Отзыв от компании “ТехноБУД”</a>
                </div>

                <div id="rev-2" class="modal fade image-modal" role="dialog">
                    <div class="modal-dialog">
                        <!-- Modal content-->
                        <div class="modal-content">
                            <div class="modal-header">
                                <a class="close" data-dismiss="modal"><i class="icon-sprite icon-close"></i></a>
                            </div>
                            <div class="modal-body">
                                <img src="img/jpeg_files/rev-2-big.jpg" alt="document">
                            </div>
                            <div class="modal-footer">
                                <p class="reviews-modal-subtitle">Сертификат соответствия всем нормам и стандартам при разработки СИЗ</p>
                            </div>
                        </div>
                    </div>
                </div>
            </li>
            <li class="three-list__item">
                <div class="reviews-list__top--wrapper">
                    <a href="#" class="reviews-list__item--image" data-toggle="modal" data-target="#rev-3">
                        <svg class="icon" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="36" height="36" viewBox="0 0 36 36" style="">
                            <image id="Shape_135_copy" data-name="Shape 135 copy" width="36" height="36" xlink:href="data:img/png;base64,"/>
                        </svg>
                        <img src="img/jpeg_files/rev-3.jpg" alt="document">
                    </a>
                </div>
                <div class="reviews-list__bottom--wrapper">
                    <a href="#" class="reviews-list__item--title">Отзыв от компании “ТехноБУД”</a>
                </div>

                <div id="rev-3" class="modal fade image-modal" role="dialog">
                    <div class="modal-dialog">
                        <!-- Modal content-->
                        <div class="modal-content">
                            <div class="modal-header">
                                <a class="close" data-dismiss="modal"><i class="icon-sprite icon-close"></i></a>
                            </div>
                            <div class="modal-body">
                                <img src="img/jpeg_files/rev-3-big.jpg" alt="document">
                            </div>
                            <div class="modal-footer">
                                <p class="reviews-modal-subtitle">Сертификат соответствия всем нормам и стандартам при разработки СИЗ</p>
                            </div>
                        </div>
                    </div>
                </div>
            </li>
            <li class="three-list__item">
                <div class="reviews-list__top--wrapper">
                    <a href="#" class="reviews-list__item--image" data-toggle="modal" data-target="#rev-1">
                        <svg class="icon" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="36" height="36" viewBox="0 0 36 36" style="">
                            <image id="Shape_135_copy" data-name="Shape 135 copy" width="36" height="36" xlink:href="data:img/png;base64,"/>
                        </svg>
                        <img src="img/jpeg_files/rev-1.jpg" alt="document">
                    </a>
                </div>
                <div class="reviews-list__bottom--wrapper">
                    <a href="#" class="reviews-list__item--title">Отзыв от компании “ТехноБУД”</a>
                </div>

                <div id="rev-1" class="modal fade image-modal" role="dialog">
                    <div class="modal-dialog">
                        <!-- Modal content-->
                        <div class="modal-content">
                            <div class="modal-header">
                                <a class="close" data-dismiss="modal"><i class="icon-sprite icon-close"></i></a>
                            </div>
                            <div class="modal-body">
                                <img src="img/jpeg_files/rev-1-big.jpg" alt="document">
                            </div>
                            <div class="modal-footer">
                                <p class="reviews-modal-subtitle">Сертификат соответствия всем нормам и стандартам при разработки СИЗ</p>
                            </div>
                        </div>
                    </div>
                </div>
            </li>
            <li class="three-list__item">
                <div class="reviews-list__top--wrapper">
                    <a href="#" class="reviews-list__item--image" data-toggle="modal" data-target="#rev-2">
                        <svg class="icon" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="36" height="36" viewBox="0 0 36 36" style="">
                            <image id="Shape_135_copy" data-name="Shape 135 copy" width="36" height="36" xlink:href="data:img/png;base64,"/>
                        </svg>
                        <img src="img/jpeg_files/rev-2.jpg" alt="document">
                    </a>
                </div>
                <div class="reviews-list__bottom--wrapper">
                    <a href="#" class="reviews-list__item--title">Сертификат соответствия всем нормам и стандартам при разработки СИЗ</a>
                </div>

                <div id="rev-2" class="modal fade image-modal" role="dialog">
                    <div class="modal-dialog">
                        <!-- Modal content-->
                        <div class="modal-content">
                            <div class="modal-header">
                                <a class="close" data-dismiss="modal"><i class="icon-sprite icon-close"></i></a>
                            </div>
                            <div class="modal-body">
                                <img src="img/jpeg_files/rev-2-big.jpg" alt="document">
                            </div>
                            <div class="modal-footer">
                                <p class="reviews-modal-subtitle">Сертификат соответствия всем нормам и стандартам при разработки СИЗ</p>
                            </div>
                        </div>
                    </div>
                </div>
            </li>
            <li class="three-list__item">
                <div class="reviews-list__top--wrapper">
                    <a href="#" class="reviews-list__item--image" data-toggle="modal" data-target="#rev-3">
                        <svg class="icon" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="36" height="36" viewBox="0 0 36 36" style="">
                            <image id="Shape_135_copy" data-name="Shape 135 copy" width="36" height="36" xlink:href="data:img/png;base64,"/>
                        </svg>
                        <img src="img/jpeg_files/rev-3.jpg" alt="document">
                    </a>
                </div>
                <div class="reviews-list__bottom--wrapper">
                    <a href="#" class="reviews-list__item--title">Сертификат лучшего поставщика</a>
                </div>

                <div id="rev-3" class="modal fade image-modal" role="dialog">
                    <div class="modal-dialog">
                        <!-- Modal content-->
                        <div class="modal-content">
                            <div class="modal-header">
                                <a class="close" data-dismiss="modal"><i class="icon-sprite icon-close"></i></a>
                            </div>
                            <div class="modal-body">
                                <img src="img/jpeg_files/rev-3-big.jpg" alt="document">
                            </div>
                            <div class="modal-footer">
                                <p class="reviews-modal-subtitle">Сертификат лучшего поставщика</p>
                            </div>
                        </div>
                    </div>
                </div>
            </li>
        </ul>
        <?php include_once "parts/pagination.php" ?>
    </div>
</section>


<?php include_once "parts/footer.php" ?>
</body>
</html>